...
enum OutputColumn {
    Key { key_idx: usize, name: PlSmallStr },
    Payload { df_idx: usize, name: PlSmallStr },
}
...
pub fn join_many_sorted(dfs: &[DataFrame], keys: &[PlSmallStr]) -> PolarsResult<DataFrame> {
    ...
    for (key_idx, key) in keys.iter().enumerate() {
        output_columns.push(OutputColumn::Key {
            key_idx,
            name: key.clone(),
        });
    }
    for (df_idx, df) in dfs.iter().enumerate() {
        for name in df.get_column_names() {
            ...
            output_columns.push(OutputColumn::Payload {
                df_idx,
                name: name.clone(),
            });
        }
    ...
    let mut matched = Vec::with_capacity(dfs.len());
    let mut matched_mask = vec![false; dfs.len()];
        ...
        .any(|(cursor, height)| *cursor < *height)
    {
        ...
        for df_idx in matched.iter().copied() {
            pre_products[df_idx] = prefix;
            let len = group_ranges[df_idx].1;
            prefix = prefix.saturating_mul(len);
        }

        let mut suffix = 1usize;
        for df_idx in matched.iter().copied().rev() {
            post_products[df_idx] = suffix;
            let len = group_ranges[df_idx].1;
            suffix = suffix.saturating_mul(len);
        }

        for (out_idx, column) in output_columns.iter().enumerate() {
            let builder = &mut builders[out_idx];
            match column {
                OutputColumn::Key { key_idx, .. } => {
                    let key_series = &key_columns[min_df][*key_idx];
                    append_key_column(builder, key_series, min_cursor, total_rows);
                },
                OutputColumn::Payload { df_idx, name } => {
                    if matched_mask[*df_idx] {
